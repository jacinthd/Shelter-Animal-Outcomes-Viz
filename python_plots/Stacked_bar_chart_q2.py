...
age_counts = pet_data.groupby(['AnimalType', 'OutcomeType', 'AgeInYears']).count().reset_index().iloc[:,:4]

# init plot area
fig, axes = plt.subplots(nrows=1, ncols=2, sharex=True, sharey=True)
# assign data to plots
labels=[]
for OutcomeType_split,intermediate in age_counts.groupby(['OutcomeType']):
    for idxAnimal, (AnimalType_split, data) in enumerate(intermediate.groupby(['AnimalType'])):
        current_ax = axes[idxAnimal]
        current_ax.bar(data['AgeInYears'], data['AnimalID'])
        current_ax.set_xticks(np.arange(0,22,step=2))
        current_ax.set_xlabel('Age in Years')
        if idxAnimal==0:
...
fig.suptitle('Number of cats/dogs by Age and split by Outcome-Type', fontsize=16)
